...
const navItems = [
  { to: '/dashboard/engineer', label: 'Dashboard', icon: LayoutDashboard },
  { to: '/bookings', label: 'My Bookings', icon: Calendar },
  { to: '/chat', label: 'Messages', icon: MessageSquare },
  { to: '/dashboard/engineer#settings', label: 'Settings', icon: Settings },
];
...
export default function EngineerDashboardLayout({
  ...
  const name = profile?.name || user?.email?.split('@')[0] || 'Engineer';
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-64 rounded-lg border border-gray-200 bg-gray-50 py-2 pl-9 pr-3 text-sm text-gray-900 placeholder:text-gray-400 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              />
            </div>
            {topAction}
          </div>
